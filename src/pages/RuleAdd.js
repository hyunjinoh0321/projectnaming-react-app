...
import { getGrade, getWork } from './api';  // get 공통 함수 : api.js
...
export default function RuleAdd() {
    ...
    useEffect(() => {
      axios.get(`http://172.23.125.110:3030/projectnaming/grade/simple`)
        .then((response) => {
          setAllGrades(response.data);
          setGrade(response.data[0].code);
        })
        .catch((error) => {
          console.log("Error while Get Grade:", error);
        });    

      axios.get(`http://172.23.125.110:3030/projectnaming/work/simple`)
        .then((response) => {
          setAllWorks(response.data);
          setWorkCode(response.data[0].workCode);
        })
        .catch((error) => {
          console.log("Error while get Work:", error);
        });  
    }, []);
    ...
    const handleSave = (event) => {
      ...
      axios.post("http://172.23.125.110:3030/projectnaming/rule", newRule)
        .then((response) => {
          console.log("Rule added successfully.");
          navigate("/projectnaming/rule/list");
        })
        .catch((error) => {
          console.log("Error while adding Rule:", error);
        });      
    }
    ...
    return (
      <div className="container">
        <h2 className="text-center mt-5 mb-3">프로젝트 네이밍 등록</h2>
        <div className="card">
          ...
          <div className="card-body">
            <form>
              <div className="form-group">
                ...
                <SelectGradeBox handle={handleGradeChange} options={allGrades} defaultValue="A">
                </SelectGradeBox>
                <input 
              ...
              <div className="form-group">
                ...
                <SelectWorkBox handle={handleWorkChange} options={allWorks} defaultValue="BPS"></SelectWorkBox>
                <input 
              ...
              <div className="form-group">
                <label htmlFor="projectName">프로젝트 네이밍</label>
                <input 
                  value="자동으로 생성됨"
                  type="text"
                  className="form-control"
